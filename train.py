...
def train():
	with tf.Graph().as_default():
		...
		validation_dataset = validation_dataset.batch(VALIDATION_BATCH_SIZE).repeat()
		...
		with tf.Session() as sess:
			...
			for i in range(FLAGS.training_steps):
				...
				if (i % 100 == 0):
					...
					val_loss = 0
					val_acc = 0
					#Compute the validation loss and accuracy by iterating over the validation set
					#in batches and summing the result
					#Log the values to view in Tensorboard later.
					for _ in range(num_validation_iters):
						val_loss += sess.run(total_loss, feed_dict={handle : val_handle}) / num_validation_iters
